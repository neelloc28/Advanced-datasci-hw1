...
**If we wanted a high true positive rate (correctly predicting cancelled reservations as cancelled) then we would want to make the cutoff for "predicted as cancelled" lower than 0.5 because that would assign more predicted probabilities to cancelled=yes.**

c. What happens to the true negative rate if we try to get a higher true positive rate?

**If we try to get a true positive rate, the true negative rate will subsequently be lower because we are classifying more reservations as "cancelled" which thereby classifies less as "not cancelled".**
